...
func PaintGif(img image.Image, writer io.Writer) {
	...
	*/
	...
	for y := 0; y < img.Bounds().Max.Y; y += int(math.Ceil(float64(img.Bounds().Bounds().Max.Y) / float64(height))) {
		for x := 0; x < img.Bounds().Max.X; x += int(math.Ceil(float64(img.Bounds().Max.X) / float64(width))) {
			var diff uint32 = 0xFFFFFFFF
